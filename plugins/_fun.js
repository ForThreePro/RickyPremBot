let handler = async (m, { conn, command, text }) => {
    if(!m.isGroup) return m.reply('╭─💚 *『 𝗥𝗜𝗖𝗞𝗬 𝗣𝗥𝗘𝗠 』* 💚─╮\n│ 😿 *Solo funciona en grupos bro*\n╰─────────────────💚')

    let metadata = await conn.groupMetadata(m.chat)
    let users = metadata.participants.map(u => u.id)
    let porcentaje = Math.floor(Math.random() * 101)

    const BOX_TOP = `╭─💚 *『 𝗥𝗜𝗖𝗞𝗬 𝗣𝗥𝗘𝗠 』* 💚─╮`
    const BOX_BOT = `╰─────────────────💚`
...
│ 📊 *NIVEL:* ${porcentaje}%
│ 💬 *DIAGNOSTICO:* ${porcentaje > 70? 'Almas Gemelas' : porcentaje > 40? 'Hay Química' : 'Frío Como Hielo'}
${BOX_BOT}`
            break
...
${BOX_BOT}`
        ...
        case '2p2': // 4 PERSONAS = 2 PAREJAS
            if(users.length < 4) return m.reply('╭─💚 *『 𝗥𝗜𝗖𝗞𝗬 𝗣𝗥𝗘𝗠 』* 💚─╮\n│ 😿 *Mínimo 4 personas en el grupo*\n╰─────────────────💚')
...
${BOX_BOT}`
        ...
        case '3p3': // 6 PERSONAS = 3 PAREJAS
            if(users.length < 6) return m.reply('╭─💚 *『 𝗥𝗜𝗖𝗞𝗬 𝗣𝗥𝗘𝗠 』* 💚─╮\n│ 😿 *Mínimo 6 personas en el grupo*\n╰─────────────────💚')
...
${BOX_BOT}`
        ...
        case 'duo': // 2 PERSONAS = 1 PAREJA
            if(users.length < 2) return m.reply('╭─💚 *『 𝗥𝗜𝗖𝗞𝗬 𝗣𝗥𝗘𝗠 』* 💚─╮\n│ 😿 *Mínimo 2 personas en el grupo*\n╰─────────────────💚')
